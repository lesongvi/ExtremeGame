/*
BlackJack от www.vk.com/pawnosos
*/
#include <a_samp>
#include <zcmd>
#include <sscanf2>
...
{
	print("\n--------------------------------------");
	print("[FilterScrips]: Дополнение RouletteGame было успешно загружено.");
	print("[FilterScrips]: Автор: www.vk.com/pawnosos");
	print("--------------------------------------\n");
...
{
    print("\n--------------------------------------");
	print("[FilterScrips]: Дополнение RouletteGame завершило свою работу.");
	print("[FilterScrips]: Автор: www.vk.com/pawnosos");
	print("--------------------------------------\n");
	return 1;
}
...
public OnDialogResponse(playerid, dialogid, response, listitem, inputtext[])
{
    if(dialogid == 5551)
	{
		...
		if(strval(inputtext) < 10000 || strval(inputtext) > 1000000) return ShowPlayerDialog(playerid, 5551, DIALOG_STYLE_INPUT, "Ставка на красный","Ввидите кол-во денег,которое хотите поставить:", "Положить", "Отмена"),SendClientMessage(playerid,-1,"Нельзя ставить меньше 10.000$ и больше 1.000.000$");
		if(GetPlayerMoney(playerid) < strval(inputtext)) return SendClientMessage(playerid,-1,"У Вас недостаточно денег.");
		betsumma[playerid] = strval(inputtext),GivePlayerMoney(playerid,-betsumma[playerid]),totalbet[0] ++,totalsumma[0] +=betsumma[playerid];
		new string[128];
		format(string,sizeof(string),"Вы успешно поставили %d$ на красный цвет.",betsumma[playerid]);
	...
	{
		if(!response) return zmidialog[playerid] = 0;
		if(strval(inputtext) < 10000 || strval(inputtext) > 1000000) return ShowPlayerDialog(playerid, 5552, DIALOG_STYLE_INPUT, "Ставка на зелёный","Ввидите кол-во денег,которое хотите поставить:", "Положить", "Отмена"),SendClientMessage(playerid,-1,"Нельзя ставить меньше 10.000$ и больше 1.000.000$");
		if(GetPlayerMoney(playerid) < strval(inputtext)) return SendClientMessage(playerid,-1,"У Вас недостаточно денег.");
		betsumma[playerid] = strval(inputtext),GivePlayerMoney(playerid,-betsumma[playerid]),totalbet[1] ++,totalsumma[1] +=betsumma[playerid];
		new string[128];
		format(string,sizeof(string),"Вы успешно поставили %d$ на зелёный цвет.",betsumma[playerid]);
		SendClientMessage(playerid,-1,string);
	...
	{
		if(!response) return zmidialog[playerid] = 0;
		if(strval(inputtext) < 10000 || strval(inputtext) > 1000000) return ShowPlayerDialog(playerid, 5553, DIALOG_STYLE_INPUT, "Ставка на чёрный","Ввидите кол-во денег,которое хотите поставить:", "Положить", "Отмена"),SendClientMessage(playerid,-1,"Нельзя ставить меньше 10.000$ и больше 1.000.000$");
		if(GetPlayerMoney(playerid) < strval(inputtext)) return SendClientMessage(playerid,-1,"У Вас недостаточно денег.");
		betsumma[playerid] = strval(inputtext),GivePlayerMoney(playerid,-betsumma[playerid]),totalbet[2] ++,totalsumma[2] +=betsumma[playerid];
		new string[128];
		format(string,sizeof(string),"Вы успешно поставили %d$ на чёрный цвет.",betsumma[playerid]);
		SendClientMessage(playerid,-1,string);
...
{
    //if(!Point(playerid, 5.0, 1954.4076,1039.7755,992.8594) || !Point(playerid, 5.0, 1950.8633,1041.6986,992.8594) || !Point(playerid, 5.0, 1956.5675,1035.7427,992.8594)) return SendClientMessage(playerid,COLOR_BLUE,""ErrorMSG"Вы находитесь далеко от столов.");
    if(RouletteInGame[playerid] == 1) return SendClientMessage(playerid,-1,"Вы уже находитесь в игре.");
...
public OnPlayerClickTextDraw(playerid, Text:clickedid)
{
	...
	{
		if(RouletteStatus == 1 || betsumma[playerid] != 0) return SendClientMessage(playerid,-1,"Вы уже сделали ставку,либо игра началась.");
		if(zmidialog[playerid] == 1) return true;
		ShowPlayerDialog(playerid, 5551, DIALOG_STYLE_INPUT, "Ставка на Красный","Ввидите кол-во денег,которое хотите поставить:", "Положить", "Отмена"),zmidialog[playerid] = 1;
	} // RED
	if(clickedid == Text:CasinoDraw[19])
	{
		if(RouletteStatus == 1 || betsumma[playerid] != 0) return SendClientMessage(playerid,-1,"Вы уже сделали ставку,либо игра началась.");
		if(zmidialog[playerid] == 1) return true;
		ShowPlayerDialog(playerid, 5552, DIALOG_STYLE_INPUT, "Ставка на Зелёный","Ввидите кол-во денег,которое хотите поставить:", "Положить", "Отмена"),zmidialog[playerid] = 1;
	} // GREEN
	if(clickedid == Text:CasinoDraw[20])
	{
		if(RouletteStatus == 1 || betsumma[playerid] != 0) return SendClientMessage(playerid,-1,"Вы уже сделали ставку,либо игра началась.");
		if(zmidialog[playerid] == 1) return true;
		ShowPlayerDialog(playerid, 5553, DIALOG_STYLE_INPUT, "Ставка на Чёрный","Ввидите кол-во денег,которое хотите поставить:", "Положить", "Отмена"),zmidialog[playerid] = 1;
	} // BLACK
	if(clickedid == Text:CasinoDraw[22])
	{
        if(zmidialog[playerid] == 1) return true;
		if(RouletteStatus == 1) return SendClientMessage(playerid,-1,"Дождитесь завершения данной игры.");
		SendClientMessage(playerid,-1,"Вы успешно покинули игру."),RouletteInGame[playerid] = 0,PlayerTextDrawSetString(playerid,CasinoDrawPlayer[playerid][1], "BET: NONE");
			...
			switch(step)
			{
				...
				case 10:
				{
                    foreach(new i:Player)
					{
                        ...
                        if(betplayer[i] == 10)
                        {
							SendClientMessage(i,-1,"Вы выиграли x2 ставку. Победил чёрный цвет.");
							GivePlayerMoney(i,betsumma[i]*2),PlayerPlaySound(i,43001,0.0,0.0,0.0);
						} else if(betplayer[i] == 14 || betplayer[i] == 4) SendClientMessage(i,-1,"Вы проиграли. Выиграл чёрный цвет."),PlayerPlaySound(i,1085,0.0,0.0,0.0);
			...
			switch(step)
			{
				...
				case 10:
				{
                    foreach(new i:Player)
					{
                        ...
                        if(betplayer[i] == 4)
                        {
							SendClientMessage(i,-1,"Вы выиграли x4 ставку. Победил зелёный цвет.");
							GivePlayerMoney(i,betsumma[i]*4),PlayerPlaySound(i,43001,0.0,0.0,0.0);
						} else if(betplayer[i] == 10 || betplayer[i] == 14) SendClientMessage(i,-1,"Вы проиграли. Выиграл зелёный цвет."),PlayerPlaySound(i,1085,0.0,0.0,0.0);
						betsumma[i] = 0,betplayer[i] = 0,PlayerTextDrawSetString(i,CasinoDrawPlayer[i][1], "BET: NONE");
					}
			...
			switch(step)
			{
				...
				case 10:
				{
                    foreach(new i:Player)
					{
                        ...
                        if(betplayer[i] == 14)
                        {
							SendClientMessage(i,-1,"Вы выиграли x2 ставку. Победил красный цвет.");
							GivePlayerMoney(i,betsumma[i]*2),PlayerPlaySound(i,43001,0.0,0.0,0.0);
						} else if(betplayer[i] == 10 || betplayer[i] == 4) SendClientMessage(i,-1,"Вы проиграли. Выиграл красный цвет."),PlayerPlaySound(i,1085,0.0,0.0,0.0);
						betsumma[i] = 0,betplayer[i] = 0,PlayerTextDrawSetString(i,CasinoDrawPlayer[i][1], "BET: NONE");
					}
					RouletteTime = 20,TotalBet = 0,RouletteStatus = 0,step = 0,step2 = 0,step3 = 0,podkrut = 0;
                    for(new i; i<3; i++) totalbet[i] = 0;
